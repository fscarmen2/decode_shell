...
xyz() {
	if [[ -n $(screen -ls | grep '(Attached)' | awk '{print $1}' | awk -F "." '{print $1}') ]]; then
		until [[ -z $(screen -ls | grep '(Attached)' | awk '{print $1}' | awk -F "." '{print $1}' | awk 'NR==1{print}') ]]; do
	...
	EOF
	[[ -e /root/WARP-UP.sh ]] && screen -ls | awk '/\.up/ {print $1}' | cut -d "." -f 1 | xargs kill 2>/dev/null
	screen -UdmS up bash -c '/bin/bash /root/WARP-UP.sh'
}
...
lncf() {
	curl -sSL -o /usr/bin/cf -L https://raw.githubusercontent.com/fscarmen2/decode_shell/main/decode-1CFwarp.sh
	chmod +x /usr/bin/cf
}
...
ShowSOCKS5() {
	if [[ $(systemctl is-active warp-svc) = active ]]; then
		...
		plus)
			S5Status=$(
				white "Socks5 WARP+状态：\c"
				rred "运行中，WARP+账户(剩余WARP+流量：$(($(warp-cli --accept-tos account | grep Quota | awk '{ print $(NF) }') / 1000000000)) GB)"
				white " Socks5 端口：\c"
				rred "$mport"
				white " 服务商 Cloudflare 获取IPV4地址：\c"
				rred "$s5ip  $country"
				white " 奈飞NF解锁情况：\c"
				rred "$NF"
				white " ChatGPT解锁情况：\c"
		...
		on)
			S5Status=$(
				white "Socks5 WARP状态：\c"
				green "运行中，WARP普通账户(无限WARP流量)"
				white " Socks5 端口：\c"
				green "$mport"
				white " 服务商 Cloudflare 获取IPV4地址：\c"
				green "$s5ip  $country"
		...
		*)
			S5Status=$(
				white "Socks5 WARP状态：\c"
				yellow "已安装Socks5-WARP客户端，但端口处于关闭状态"
			)
			;;
		esac
	else
...
ONEWARPGO() {
		...
		if [[ -n $v6 ]]; then
			...
			plus)
				WARPIPv6Status=$(
					white "WARP+状态：\c"
					rred "运行中，$cfplus"
					white " 服务商 Cloudflare 获取IPV6地址：\c"
					rred "$v6  $country"
					white " 奈飞NF解锁情况：\c"
					rred "$NF"
					white " ChatGPT解锁情况：\c"
	...
	WGCFins() {
		if [[ $release = Centos ]]; then
			...
			yum install iproute iputils -y
		elif [[ $release = Debian ]]; then
			apt install lsb-release -y
			echo "deb http://deb.debian.org/debian $(lsb_release -sc)-backports main" | tee /etc/apt/sources.list.d/backports.list
			apt update -y
			apt install iproute2 openresolv dnsutils iputils-ping -y
		elif [[ $release = Ubuntu ]]; then
			apt update -y
			apt install iproute2 openresolv dnsutils iputils-ping -y
		fi
...
ONEWGCFWARP() {
	...
	WGCFins() {
		...
		if [[ $release = Centos ]]; then
			...
			echo "deb http://deb.debian.org/debian $(lsb_release -sc)-backports main" | tee /etc/apt/sources.list.d/backports.list
			apt update -y
			apt install iproute2 openresolv dnsutils iptables iputils-ping -y
