...
sbymfl() {
	...
	[[ -n $(curl -4sx socks5h://localhost:40000 icanhazip.com -k) ]] && warp_s4_ip="当前IP：$(curl -4sx socks5h://localhost:40000 icanhazip.com -k)" || warp_s4_ip='无warp-s5的IPV4，黑名单模式'
	[[ -n $(curl -6sx socks5h://localhost:40000 icanhazip.com -k) ]] && warp_s6_ip="当前IP：$(curl -6sx socks5h://localhost:40000 icanhazip.com -k)" || warp_s6_ip='无warp-s5的IPV6，黑名单模式'
	v4v6
	if [[ -z $v4 ]]; then
...
lnsb() {
	curl -sL -o /usr/bin/sb https://raw.githubusercontent.com/fscarmen2/decode_shell/main/decode-1sb.sh
	chmod +x /usr/bin/sb
}
...
unins() {
	...
	service iptables save >/dev/null 2>&1
	green "Sing-box卸载完成！"
	blue "欢迎继续使用Sing-box-yg脚本：bash <(curl -Ls https://gitlab.com/rwkgyg/sing-box-yg/raw/main/sb.sh)"
	echo
}
